...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self,
        ...
        # gumbel 和 temperature 的作用： Gumbel_MAX
    ...
    code_book
        -- code: self.embedding
        -- label: default_label, 0: cluster_center, 1: feature
        -- semantic_label: [0, M)
    """
    ...
    def get_codebook(self):
        ...
        default_label = torch.zeros(self.n_e).to(self.device)
        default_label[self.n_cluster:] = 1
        codes['label'] = default_label
        l_c = torch.arange(end=self.n_cluster)
        l_f = torch.randint(0, self.n_cluster, size=(self.n_e - self.n_cluster, 1)).squeeze()
        codes['semantic_label'] = torch.cat((l_c, l_f), dim=0).to(self.device)
        self.codebook = codes
    ...
    def update_codebook(self):
        if self.n_cluster != 0:
            d = self.get_distance(self.embedding[self.n_cluster:], 'center')
            label_ = torch.argmax(-d, dim=1)
            self.codebook['semantic_label'][self.n_cluster:] = label_
        # for i in range(self.n_cluster):
        #     idx = self.codebook['semantic_label'] == i
        #     print(i, '   ', idx.sum())
    ...
    def get_index(self, logits, topk=1, step=None, total_steps=None):
        """
        logits: BHW x N， logits means distance
        ...
        if self.gumbel_sample:  # self.gumbel_sample = False
            gumbel = True
            if self.training:
        ...
        if topk == 1:
            ...
            # 从 logits 中选出 top_k 个 token
            _, indices = torch.topk(logits, dim=1, k=topk)  # N x K, _: values
            # 创建 N 个范围在 [0, K) 的随机数 -> N x 1
            chose = torch.randint(0, topk, (indices.shape[0],)).to(indices.device)  # N
            # scatter_() 方法将 chose 映射到 N x K 与 indices 对应
            chose = torch.zeros_like(indices).scatter_(1, chose.unsqueeze(dim=1), 1.0)  # N x K
            # 从 N x K 个 token 中选出最终的 N x 1 个 （K 选 1）
            indices = (indices * chose).sum(dim=1, keepdim=False)  # N
    ...
    def get_distance(self, z, code_type='all'):
        ...
        return:
            d = (z) * (embedding T)
            d: L x N, where N = n_e or M or (n_e - M), M is n_cluster
        """
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        """
            z: L x D, L = BHW
            token_type: L, 0 denote cluster center token, other feature token
        """
        d = self.get_distance(z)  # L x N, 每一行代表 1 个像素和 N 个 embedding 的距离
        # print(d[:10,:15])
        ...
        if token_type is None or self.n_cluster == 0:
            min_encoding_indices = self.get_index(d, topk=topk, step=step, total_steps=total_steps)
            token_semantic_type = torch.zeros(z.shape[0]).to(z.device)
        else:
            # min_encoding_indices = self.get_index(d, topk=topk, step=step, total_steps=total_steps)
            # token_semantic_type = torch.zeros(z.shape[0]).to(z.device)

            min_encoding_indices = torch.zeros(z.shape[0]).long().to(z.device)  # L
            with torch.no_grad():
                # update token_semantic_type，通过计算与 m 个 cluster center的距离得到
                self.update_codebook()
                d_from_center = d[:, :self.n_cluster]  # L x M
                d_from_center = torch.sigmoid(-torch.log(d_from_center + 1)) + torch.log(get_gaussian_weight(self.n_cluster).to(z.device) + 1)
                token_semantic_type = torch.argmax(d_from_center, dim=1)
            # 通过 token_semantic_type == m 判断 d 中哪些行需要被取出（即属于第 m 个语义的embedding），
            # 在下一步 get_index() 中从这些行中得到最近的 top_k 中的随机一个
            for i in range(self.n_cluster):
                idx1 = token_semantic_type == i
                idx2 = self.codebook['semantic_label'] == i
                if idx1.sum() > 0:
                    d_ = d[idx1][:, idx2]  # d_: p x m', p∈[0,BHW], m'∈[0,n_e-m]
        ...
        print(min_encoding_indices[10:20])

        return z_q, min_encoding_indices, token_semantic_type

    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        """
            z: B x C x H x W
            token_type: B x 1 x H x W, for divide masked and unmasked region
        """
        ...
        z_q, min_encoding_indices, token_semantic_type = self._quantize(z_flattened, token_type=token_type_flattened,
                                                                        topk=topk, step=step, total_steps=total_steps)
        ...
        if self.training and self.embed_ema:
            ...
            z_sum = (z_flattened.transpose(0, 1) @ indices_onehot).transpose(0, 1)  # n_e x C (or e_dim)
            # z_sum[i] = ∑ z(i,j) j∈[1,ni]
            # ni 表示有 n 个 z 属于第 i 个 embedding
        ...
        # reshape back to match original input shape B x C x H x W
        z_q = z_q.permute(0, 3, 1, 2).contiguous()
        unique_idx = min_encoding_indices.unique()
        output = {
            'quantize': z_q,
            # 'token_semantic_type': str([torch.sum(token_semantic_type == i) for i in range(self.n_cluster)]),
...
class PatchVQGAN(BaseCodec):
    def __init__(self,
                 *,
                 ...
                 quantizer_config=None,  # quantizer can be given by this config (strongly recommended),
    ...
    def get_codebook_entry_with_token(self, token, **kwargs):
        ...
        """
        print('get_codebook_entry_with_token!!!!!!!!!!!!!!!!!')
        t_shape = token.shape
    ...
    def decode(self, token, mask_im=None, mask=None, combine_rec_and_gt=True, token_shape=None):
        """
        Decode the image with provided tokens, tokens(indices)
        """
        print('decode!!!!!!!!!!!!!!!!!')
    ...
    def forward(self, batch, name='none', return_loss=True, step=0, total_steps=None, **kwargs):
        ...
        if name == 'generator':
            ...
            input_vq = img * batch['erase_mask']
            x = self.encoder(input_vq)
            x = self.quant_conv(x)
            ...
            if self.decoder.requires_image:
                rec = self.decoder(quant, self.multi_pixels_with_mask(img, batch['mask']), mask=batch['mask'])
            else:
                rec = self.decoder(quant)
            ...
            for k, v in quant_out.items():
                if k == 'loss':
                    continue
                if k == 'token_semantic_type':
                    output[k] = v
                if k == 'used_quantize_embed':
                    output[k] = v
