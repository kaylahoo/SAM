...
"""
svq_cosineDis_GTinput_referForUnmasked_learnedClsViaKL
loss between sem type and embed sem label
"""
...
def calculate_classifier_loss(q, p):
    q = q.sum(dim=0).to(torch.float32) / q.shape[0]
    p = p.sum(dim=0).to(torch.float32) / p.shape[0]
...
class VectorQuantizer(nn.Module):
    ...
    def update_semantic_label(self):
        if self.n_cluster > 0:
            if self.semantic_label is None:
                ...
                label_ = torch.argmax(-d[:, :self.n_cluster], dim=1)
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        if self.n_cluster == 0:
            ...
            d_from_center = self.alpha * torch.abs(sem_cls) - (1 - self.alpha) * d_from_center
            ...
            token_semantic_type = F.gumbel_softmax(d_from_center, hard=True)    # one-hot # P(z|k)
            embed_semantic_label = F.one_hot(self.semantic_label)   # P(k)
            cls_loss = calculate_classifier_loss(token_semantic_type, embed_semantic_label)
            selector = token_semantic_type.to(torch.float16) @ embed_semantic_label.to(torch.float16).t()
            d_ = d * selector
        ...
        return z_q, min_encoding_indices, cls_loss
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        z_q, min_encoding_indices, cls_loss = self._quantize(z_flattened, token_type=token_type_flattened, topk=topk, step=step,
                                                   total_steps=total_steps)
        ...
        output = {
            'quantize': z_q,
            'used__quantize_embed': torch.zeros_like(loss) + (unique_idx >= 0).sum(),
            'quantize_loss': loss,
            'cls_loss': cls_loss,
...
get_tokens_with_feature(feat, ...): return output of VectorQuantizer
get_token()
"""
