...
class VectorQuantizer(nn.Module):
    ...
    def get_distance(self, z, code_type='all'):
        ...
        if self.distance_type == 'euclidean':
            ...
            norm = torch.sqrt(torch.sum(z ** 2, dim=1).unsqueeze(dim=1) * torch.sum(self.embedding ** 2, dim=1).unsqueeze(dim=0))
            d = 0 - (0.5 + 0.5 * d / norm)
        else:
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        z_q, min_encoding_indices = self._quantize(z_flattened, token_type=token_type_flattened, topk=topk, step=step,
                                                   total_steps=total_steps)
...
class EncoderInPatchConvDecoder2(nn.Module):
    def __init__(self, in_ch, up_layers=None, with_res_block=True, res_block_bottleneck=2, downsample_layer='downsample',
                 partial_conv=False):
        super().__init__()
...
class PatchConvDecoder2(nn.Module):
    ...
    def forward(self, x, masked_image=None, mask=None):
        ...
        if self.up_layer_with_image:
            ...
            x = x * mask + im_x[str(tuple(x.shape))] * (1 - mask)
            x = self.post_layers(x)
        else:
            x = self.up_layers(x)
            x = self.post_layers(x)
