...
class VectorQuantizer(nn.Module):
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        if self.n_cluster == 0:
            ...
            d_from_center = self.alpha * torch.abs(sem_cls) - (1 - self.alpha) * d_from_center  # L x k
            ...
            dis_q = (torch.sum(token_semantic_type, dim=0) / z.shape[0]) * (torch.sum(embed_semantic_label, dim=0) / self.n_e)
            cls_loss = calculate_kl_loss(dis_q, dis_p)
