    ...
    def forward(self,
        ...
        if name == 'generator':
            ...
            if self.g_rec_loss_weight > 0:
                gen_rec_loss_masked = self.l1_loss(reconstruction, image) * self.g_rec_loss_weight / (torch.mean(mask) + 1e-6)
                gen_rec_loss_unmasked = self.l1_loss(reconstruction, image) * self.g_rec_loss_weight / (torch.mean(1 - mask) + 1e-6)
                out['rec_loss'] = gen_rec_loss_masked + gen_rec_loss_unmasked
                gen_loss = gen_loss + gen_rec_loss_masked + gen_rec_loss_unmasked
